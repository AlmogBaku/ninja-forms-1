...
registerBlockType("ninja-forms/submissions-table", {
	...
	edit: withSelect(select => {
		...
		if (!props.attributes.formId) {
			if (!props.forms)
				return (
					<Placeholder label={__("Ninja Forms Submissions Table")}>
						<Spinner />
					</Placeholder>
				);
		...
		const WithFormFields = withSelect(select => ({
			fields: select("ninja-forms-views").getFormFields(
				props.attributes.formId
			),
			submissions: select("ninja-forms-views").getFormSubmissions(
				props.attributes.formId
			)
		}))(({ fields, submissions }) => {
			return fields && submissions ? (
				...
				<Placeholder label={__("Loading Form Data")}>
					<Spinner />
				</Placeholder>
			);
		});
