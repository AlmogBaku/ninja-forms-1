...
export default ({formId, selectedFields, fields, submissions}) => {
    ...
    const columns = selectedFormFields.map((field) => (
        { Header: field.label, accessor: field.id.toString() }
    ))
    ...
    const loadMore = (nextPageIndex) => {
        ...
        select('ninja-forms-views').getFormSubmissionsPage(formId, nextPageIndex + 1)
    }
 ...
 */
function Table({ columns, data, initialPageIndex, loadMore, more, loading }) {
  ...
  return (
    <>
      ...
      <Pagination {...instance} loadMore={() => loadMore(pageIndex+1)} more={more} loading={loading} />
    </>
  )
}
...
function Pagination({previousPage, nextPage, canPreviousPage, canNextPage, loadMore, more, loading}) {
    const Style = {
        display: "flex",
